...
namespace AdvDisplays {
    ...
    /********** OLED Korean (Hangul Support) **********/

    // Korean font buffer (16x16 pixels per character)
    let _koreanBuffer: Buffer = null
    let _koreanBufferWidth: number = 16
    let _koreanBufferHeight: number = 16

    // Choseong (Initial consonants) - 19 characters: ㄱㄲㄴㄷㄸㄹㅁㅂㅃㅅㅆㅇㅈㅉㅊㅋㅌㅍㅎ
    // 8x8 top-left positioned bitmaps
    const HANGUL_CHO: number[][] = [
        [0x00, 0x7C, 0x04, 0x04, 0x04, 0x04, 0x7C, 0x00],  // ㄱ
        [0x7C, 0x04, 0x7C, 0x00, 0x7C, 0x04, 0x7C, 0x00],  // ㄲ
        [0x00, 0x7C, 0x40, 0x40, 0x40, 0x40, 0x7C, 0x00],  // ㄴ
        [0x00, 0x7C, 0x44, 0x44, 0x44, 0x44, 0x7C, 0x00],  // ㄷ
        [0x7C, 0x44, 0x7C, 0x00, 0x7C, 0x44, 0x7C, 0x00],  // ㄸ
        [0x00, 0x7C, 0x04, 0x7C, 0x40, 0x40, 0x7C, 0x00],  // ㄹ
        [0x00, 0x7C, 0x54, 0x54, 0x54, 0x54, 0x7C, 0x00],  // ㅁ
        [0x00, 0x7C, 0x54, 0x54, 0x54, 0x5C, 0x40, 0x00],  // ㅂ
        [0x7C, 0x54, 0x5C, 0x40, 0x7C, 0x54, 0x5C, 0x00],  // ㅃ
        [0x00, 0x44, 0x44, 0x28, 0x28, 0x10, 0x10, 0x00],  // ㅅ
        [0x44, 0x28, 0x10, 0x00, 0x44, 0x28, 0x10, 0x00],  // ㅆ
        [0x00, 0x38, 0x44, 0x44, 0x44, 0x44, 0x38, 0x00],  // ㅇ
        [0x00, 0x7C, 0x10, 0x28, 0x28, 0x44, 0x44, 0x00],  // ㅈ
        [0x7C, 0x10, 0x28, 0x44, 0x7C, 0x10, 0x28, 0x00],  // ㅉ
        [0x00, 0x7C, 0x10, 0x28, 0x44, 0x44, 0x7C, 0x00],  // ㅊ
        [0x00, 0x7C, 0x04, 0x04, 0x04, 0x04, 0x04, 0x00],  // ㅋ
        [0x00, 0x7C, 0x44, 0x44, 0x7C, 0x44, 0x44, 0x00],  // ㅌ
        [0x00, 0x7C, 0x54, 0x54, 0x5C, 0x40, 0x7C, 0x00],  // ㅍ
        [0x00, 0x44, 0x44, 0x7C, 0x44, 0x44, 0x7C, 0x00]   // ㅎ
    ]

    // Jungseong (Vowels) - 21 characters: ㅏㅐㅑㅒㅓㅔㅕㅖㅗㅘㅙㅚㅛㅜㅝㅞㅟㅠㅡㅢㅣ
    // Type: 0=vertical right, 1=vertical right double, 2=horizontal bottom, 3=complex
    const HANGUL_JUNG_TYPE: number[] = [0, 1, 0, 1, 0, 1, 0, 1, 2, 3, 3, 3, 2, 2, 3, 3, 3, 2, 2, 3, 0]
    const HANGUL_JUNG: number[][] = [
        [0x10, 0x10, 0x10, 0xF0, 0x10, 0x10, 0x10, 0x10],  // ㅏ
        [0x10, 0x10, 0xF0, 0x10, 0xF0, 0x10, 0x10, 0x10],  // ㅐ
        [0x50, 0x50, 0x50, 0xD0, 0x50, 0x50, 0x50, 0x50],  // ㅑ
        [0x50, 0x50, 0xD0, 0x50, 0xD0, 0x50, 0x50, 0x50],  // ㅒ
        [0x10, 0x10, 0x10, 0x1E, 0x10, 0x10, 0x10, 0x10],  // ㅓ
        [0x10, 0x10, 0x1E, 0x10, 0x1E, 0x10, 0x10, 0x10],  // ㅔ
        [0x14, 0x14, 0x14, 0x16, 0x14, 0x14, 0x14, 0x14],  // ㅕ
        [0x14, 0x14, 0x16, 0x14, 0x16, 0x14, 0x14, 0x14],  // ㅖ
        [0x00, 0x00, 0x00, 0xFE, 0x10, 0x10, 0x10, 0x00],  // ㅗ
        [0x00, 0x20, 0xFE, 0x20, 0xFE, 0x08, 0x08, 0x00],  // ㅘ
        [0x20, 0xFE, 0x20, 0xFE, 0x08, 0xFE, 0x08, 0x00],  // ㅙ
        [0x00, 0x10, 0xFE, 0x10, 0x10, 0xFE, 0x10, 0x00],  // ㅚ
        [0x00, 0x28, 0x28, 0xFE, 0x10, 0x10, 0x10, 0x00],  // ㅛ
        [0x00, 0x10, 0x10, 0x10, 0xFE, 0x00, 0x00, 0x00],  // ㅜ
        [0x00, 0x08, 0xFE, 0x08, 0xFE, 0x20, 0x20, 0x00],  // ㅝ
        [0x08, 0xFE, 0x08, 0xFE, 0x20, 0xFE, 0x20, 0x00],  // ㅞ
        [0x00, 0x10, 0xFE, 0x10, 0x10, 0xFE, 0x10, 0x00],  // ㅟ
        [0x00, 0x28, 0x28, 0x28, 0xFE, 0x00, 0x00, 0x00],  // ㅠ
        [0x00, 0x00, 0x00, 0xFE, 0x00, 0x00, 0x00, 0x00],  // ㅡ
        [0x00, 0x10, 0x10, 0xFE, 0x00, 0xFE, 0x00, 0x00],  // ㅢ
        [0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10]   // ㅣ
    ]

    // Jongseong (Final consonants) - 27 characters (index 0 = no final)
    // ㄱㄲㄳㄴㄵㄶㄷㄹㄺㄻㄼㄽㄾㄿㅀㅁㅂㅄㅅㅆㅇㅈㅊㅋㅌㅍㅎ
    const HANGUL_JONG: number[][] = [
        [0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00],  // (none)
        [0x00, 0x7C, 0x04, 0x04, 0x04, 0x7C, 0x00, 0x00],  // ㄱ
        [0x7C, 0x04, 0x7C, 0x7C, 0x04, 0x7C, 0x00, 0x00],  // ㄲ
        [0x7C, 0x04, 0x7C, 0x44, 0x28, 0x10, 0x00, 0x00],  // ㄳ
        [0x00, 0x7C, 0x40, 0x40, 0x40, 0x7C, 0x00, 0x00],  // ㄴ
        [0x7C, 0x40, 0x7C, 0x10, 0x28, 0x44, 0x00, 0x00],  // ㄵ
        [0x7C, 0x40, 0x7C, 0x44, 0x7C, 0x44, 0x00, 0x00],  // ㄶ
        [0x00, 0x7C, 0x44, 0x44, 0x44, 0x7C, 0x00, 0x00],  // ㄷ
        [0x00, 0x7C, 0x04, 0x7C, 0x40, 0x7C, 0x00, 0x00],  // ㄹ
        [0x7C, 0x04, 0x7C, 0x40, 0x7C, 0x04, 0x7C, 0x00],  // ㄺ
        [0x7C, 0x04, 0x7C, 0x40, 0x7C, 0x54, 0x7C, 0x00],  // ㄻ
        [0x7C, 0x04, 0x7C, 0x40, 0x7C, 0x54, 0x5C, 0x00],  // ㄼ
        [0x7C, 0x04, 0x7C, 0x40, 0x7C, 0x28, 0x10, 0x00],  // ㄽ
        [0x7C, 0x04, 0x7C, 0x40, 0x7C, 0x44, 0x7C, 0x00],  // ㄾ
        [0x7C, 0x04, 0x7C, 0x40, 0x7C, 0x54, 0x7C, 0x00],  // ㄿ
        [0x7C, 0x04, 0x7C, 0x40, 0x7C, 0x44, 0x7C, 0x00],  // ㅀ
        [0x00, 0x7C, 0x54, 0x54, 0x54, 0x7C, 0x00, 0x00],  // ㅁ
        [0x00, 0x7C, 0x54, 0x54, 0x54, 0x5C, 0x40, 0x00],  // ㅂ
        [0x7C, 0x54, 0x5C, 0x40, 0x44, 0x28, 0x10, 0x00],  // ㅄ
        [0x00, 0x44, 0x44, 0x28, 0x28, 0x10, 0x00, 0x00],  // ㅅ
        [0x44, 0x28, 0x10, 0x00, 0x44, 0x28, 0x10, 0x00],  // ㅆ
        [0x00, 0x38, 0x44, 0x44, 0x44, 0x38, 0x00, 0x00],  // ㅇ
        [0x00, 0x7C, 0x10, 0x28, 0x44, 0x44, 0x00, 0x00],  // ㅈ
        [0x00, 0x7C, 0x10, 0x28, 0x44, 0x7C, 0x00, 0x00],  // ㅊ
        [0x00, 0x7C, 0x04, 0x04, 0x04, 0x04, 0x00, 0x00],  // ㅋ
        [0x00, 0x7C, 0x44, 0x7C, 0x44, 0x44, 0x00, 0x00],  // ㅌ
        [0x00, 0x7C, 0x54, 0x5C, 0x40, 0x7C, 0x00, 0x00],  // ㅍ
        [0x00, 0x44, 0x7C, 0x44, 0x44, 0x7C, 0x00, 0x00]   // ㅎ
    ]

    // Decompose Hangul syllable into Jamo indices
    function decomposeHangul(code: number): number[] {
        if (code < 0xAC00 || code > 0xD7A3) {
            return [-1, -1, -1]  // Not a Hangul syllable
        }
        let syllableIndex = code - 0xAC00
        let cho = Math.floor(syllableIndex / (21 * 28))
        let jung = Math.floor((syllableIndex % (21 * 28)) / 28)
    ...
    // Render a single Hangul character to buffer
    function renderHangulChar(cho: number, jung: number, jong: number): Buffer {
        let buf = pins.createBuffer(32)  // 16x16 = 256 bits = 32 bytes
        for (let i = 0; i < 32; i++) buf[i] = 0

        let jungType = HANGUL_JUNG_TYPE[jung]
        let hasJong = jong > 0

        // Determine layout based on vowel type
        if (jungType == 0 || jungType == 1) {
            // Vertical vowel (ㅏㅐㅑㅒㅓㅔㅕㅖㅣ) - cho on left, jung on right
            // Cho: top-left 8x8 (or 8x6 if jong exists)
            let choHeight = hasJong ? 5 : 8
            for (let row = 0; row < choHeight; row++) {
                let choData = HANGUL_CHO[cho][row]
                let byteIndex = row * 2
                buf[byteIndex] = choData
            }
            // Jung: right side
            let jungHeight = hasJong ? 10 : 16
            for (let row = 0; row < 8 && row < jungHeight; row++) {
                let jungData = HANGUL_JUNG[jung][row]
                let byteIndex = row * 2 + 1
                buf[byteIndex] |= (jungData >> 4) & 0x0F
            }
            // Jong: bottom 8x6
            if (hasJong) {
                for (let row = 0; row < 6; row++) {
                    let jongData = HANGUL_JONG[jong][row]
                    let byteIndex = (10 + row) * 2
                    buf[byteIndex] = jongData
                }
            }
        } else if (jungType == 2) {
            // Horizontal vowel (ㅗㅛㅜㅠㅡ) - cho on top, jung below
            // Cho: top center 8x5
            ...
            // Jung: middle horizontal
            let jungStartRow = hasJong ? 5 : 8
            for (let row = 0; row < 4; row++) {
                let jungData = HANGUL_JUNG[jung][row]
                let byteIndex = (jungStartRow + row) * 2
                buf[byteIndex] = jungData
                buf[byteIndex + 1] = jungData
            }
            // Jong: bottom
            if (hasJong) {
                for (let row = 0; row < 6; row++) {
            ...
            // Complex vowel (ㅘㅙㅚㅝㅞㅟㅢ) - combination layout
            // Cho: top-left 6x5
            for (let row = 0; row < 5; row++) {
                let choData = HANGUL_CHO[cho][row]
                let byteIndex = row * 2
            ...
            // Jung: complex - spread across
            for (let row = 0; row < 8; row++) {
                let jungData = HANGUL_JUNG[jung][row]
                let byteIndex = (row < 5 ? row : row + 2) * 2 + 1
                if (byteIndex < 32) {
                    buf[byteIndex] |= (jungData >> 4) & 0x0F
                }
                if (row >= 5) {
                    let byteIndex2 = (row + 2) * 2
                    if (byteIndex2 < 32) {
                        buf[byteIndex2] = jungData
                    }
                }
            }
            // Jong: bottom
    ...
    // Draw 16x16 Korean character buffer to OLED
    function drawKoreanCharToOLED(x: number, y: number, charBuf: Buffer, color: OLEDColor): void {
        for (let row = 0; row < 16; row++) {
            let byteIndex = row * 2
            let data1 = charBuf[byteIndex]
            let data2 = charBuf[byteIndex + 1]

            for (let col = 0; col < 8; col++) {
                if (data1 & (0x80 >> col)) {
                    oledSetPixel(x + col, y + row, color)
                } else if (color == OLEDColor.White) {
                    oledSetPixel(x + col, y + row, OLEDColor.Black)
                }
            }
            for (let col = 0; col < 8; col++) {
                if (data2 & (0x80 >> col)) {
                    oledSetPixel(x + 8 + col, y + row, color)
                } else if (color == OLEDColor.White) {
                    oledSetPixel(x + 8 + col, y + row, OLEDColor.Black)
                }
            }
        }
    }

    //% block="OLED Korean show text %text at x %x y %y color %color"
    //% x.defl=0 y.defl=0
    //% group="OLED Korean" weight=79
    //% inlineInputMode=inline
    export function oledShowKorean(text: string, x: number, y: number, color: OLEDColor): void {
        let cursorX = x
        let cursorY = y
        ...
        for (let i = 0; i < text.length; i++) {
            ...
            if (isHangulSyllable(code)) {
                // Korean character - 16x16
                let jamo = decomposeHangul(code)
                let charBuf = renderHangulChar(jamo[0], jamo[1], jamo[2])
                drawKoreanCharToOLED(cursorX, cursorY, charBuf, color)
                cursorX += 16
            } else if (code == 32) {
                // Space
                cursorX += 8
            } else if (code >= 33 && code <= 127) {
                // ASCII - use existing 5x7 font
                oledSetCursor(cursorX, cursorY)
                oledWriteChar(code, color)
                cursorX += 6
            }

            // Line wrap
            if (cursorX >= _oledWidth - 16) {
                cursorX = x
                cursorY += 16
            }
        }
    ...
    //% block="OLED Korean show single char %char at x %x y %y color %color"
    //% x.defl=0 y.defl=0
    //% group="OLED Korean" weight=78
    //% inlineInputMode=inline
    export function oledShowKoreanChar(char: string, x: number, y: number, color: OLEDColor): void {
        ...
        if (isHangulSyllable(code)) {
            let jamo = decomposeHangul(code)
            let charBuf = renderHangulChar(jamo[0], jamo[1], jamo[2])
            drawKoreanCharToOLED(x, y, charBuf, color)
        }
    }

    //% block="OLED Korean clear area x %x y %y width %w height %h"
    //% x.defl=0 y.defl=0 w.defl=128 h.defl=16
    //% group="OLED Korean" weight=77
    //% inlineInputMode=inline
    export function oledKoreanClearArea(x: number, y: number, w: number, h: number): void {
    ...
    //% block="is Hangul character %char"
    //% group="OLED Korean" weight=76
    export function isHangul(char: string): boolean {
        if (char.length == 0) return false
        return isHangulSyllable(char.charCodeAt(0))
    }
